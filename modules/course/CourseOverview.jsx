...
const CourseOverview = ({dict}) => {
    return (
        <section className={"pt-5 pb-20 container"}>
            <HeadingSection>{dict.courseOverview.title}</HeadingSection>
                ...
                <Card>
                    <CardContent className={"shadow-card p-4 sm:pb-24 mb-2"}>
                        <TypographyLead className={"text-center"}>
                            {dict.courseOverview.description}
                        </TypographyLead>
                    </CardContent>
                </Card>
                <div className={"sm:absolute bottom-0 right-1/2 sm:translate-y-1/2 sm:translate-x-1/2 w-full grid grid-cols-2 sm:flex sm:items-center sm:justify-between gap-2"}>
