...
const CourseHead = ({dict}) => {
    return (
        <header className={"head-bg md:h-[calc(100dvh-91px)]"}>
                ...
                <div className={"flex pt-[136px] items-center h-full flex-col md:flex-row"}>
                    <div className={"capitalize h-fit flex flex-col gap-4"}>
                        ...
                        <span className={"flex gap-2 items-center text-white"}>
                            <span className={"bg-white/10 py-1 px-2 rounded"}>{dict.courseHead.foundational}</span>
                            <span className={"flex items-center gap-1"}>4.5/5 <Rating value={4.5} /></span>
                            <span>(1966)</span>
                        </span>
                        <TypographyH1 className={"text-white font-bold mb-4 leading-none"}>Medical Coding and Transcription</TypographyH1>
                        <div className={'flex flex-col gap-4'}>
                            <TypographyLead className={"text-white mb-2"}>
                                <span className={"text-[--secondary-color]"}>{dict.courseHead.with}</span> DR.Nabil Makram
                            </TypographyLead>
                            <p className={"bg-white/10 text-[--secondary-color] py-1 px-2 rounded text-[16px] w-fit"}>
                                {dict.courseHead.coFounder}
                            </p>
                            <TypographyLead className={"text-white"}>
                                {dict.courseHead.courseDescription}
                            </TypographyLead>
                            <span className={"flex items-center text-white gap-4"}>
                                <span className={"flex items-center gap-2"}>
                                    <Image src={iconWeek} alt={"duration"} />
                                    {dict.courseHead.weeks}
                                </span>
                                <span className={"flex items-center gap-2"}>
                                    <Image src={videoLesson} alt={"duration"} />
                                    {dict.courseHead.lessons}
                                </span>
                            </span>
                            <span className={"flex items-center gap-3"}>
                                <Button className={"bg-transparent py-2 px-3 text-lg border-2 border-[--secondary-color] w-fit hover:bg-transparent"}>
                                    {dict.courseHead.startCourse}
                                </Button>
                                <TypographyMuted className={"text-white"}>
                                    {dict.courseHead.freeDays}
                                </TypographyMuted>
                            </span>
                        </div>
                    </div>
